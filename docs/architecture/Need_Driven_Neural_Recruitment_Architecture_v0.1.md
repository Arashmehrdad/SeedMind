...
92. Restart-equivalence proof for the long-horizon experiment is explicitly deferred to Batch 2; Batch 1 must instead prove deterministic rerun equality, canonical ASCII JSON snapshot stability, and SHA-256 identity stability without touching main brain persistence or standalone acceptance payloads.
93. Batch 1 timelines, memory snapshots, and source-ledger evidence must be transitively immutable and must be semantically revalidated for exact schedule, memory continuity, derived per-family scores, replay summaries, structural invariants, aggregate pass criteria, canonical snapshot, and identity before acceptance.
