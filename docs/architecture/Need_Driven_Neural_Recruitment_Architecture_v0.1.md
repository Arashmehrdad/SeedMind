...
67. Training-time safe-experiment permission review must consume one complete revalidated Batch 6 proposal result, one exact expected proposal identity, one exact expected permission requirement, and one explicit human approve, reject, or defer action.
...
90. Long-horizon mixed-task interference Batch 1 must remain a separate deterministic in-memory experiment with exactly three conditions, exactly one family A source ledger and consolidation candidate, exactly thirty-six task-family steps in stable repeating order `A, B, A, C`, and family A steps limited to retention probes that do not mutate memory or source evidence.
91. Batch 1 replay may occur only after a family B or C learning step drives family A below the configured retention threshold, may replay at most one exact family A source event per learning step and within a finite total cap, and may not create new source traces, alter the old-family mastery profile, cause structural growth, use SQLite cognition, or grant action-selection, recommendation, scheduling, execution, live-integration, promotion, or production authority.
92. Long-horizon mixed-task interference Batch 2 must remain an isolated store and restart proof for exactly one validated Batch 1 `LongHorizonInterferenceResult`; it must not persist the richer evidence wrapper, alter Batch 1 numeric behaviour or public names, or change main brain persistence, standalone acceptance persistence, bounded imagination persistence, live integration, scheduling, execution, recommendation, promotion, or production action authority.
93. Batch 2 load must accept only exact canonical payload fields, exact enum values, finite numeric types, immutable tuple reconstruction, exact memory-route ordering, exact thirty-six-step `A, B, A, C` schedule contracts, exact consolidation and mastery restoration, canonical snapshot rederivation, SHA-256 identity rederivation, and full semantic revalidation; malformed, non-ASCII, unknown-field, checksum-mismatched, identity-mismatched, semantically tampered, or incompatible-schema data must fall back to an explicit non-proof state with `result=None`, no partial proof, and no authority.
94. Batch 2 restart proof must remain separate from the stored Batch 1 result, preserve `restart_proof_included=False` on the source, restored, and rerun results, preserve zero authority and zero deltas, require exact reload equality plus deterministic post-restart rerun equality, and report unchanged bounded-imagination non-persistence, unchanged main-brain schema, and unchanged standalone-acceptance schema before passing.
95. Batch 1 timelines, memory snapshots, and source-ledger evidence must be transitively immutable and must be semantically revalidated for exact schedule, memory continuity, derived per-family scores, replay summaries, structural invariants, aggregate pass criteria, canonical snapshot, and identity before acceptance.
96. Standalone NDNRA v0.1 closes only under an explicit bounded scope: one active need pulse at a time, grounded exact context signatures, fixed bounded inspectable transfer weights, specialist-interaction growth rather than unrestricted generic neuron creation, and no semantic abstraction or simultaneous-need arbitration claim.
97. Final v0.1 closure requires all three retained capabilities to be complete and evidenced: normalized competing recruitment, graph-locally derived representational saturation, and long-horizon mixed-task interference plus isolated restart proof.
98. Standalone acceptance persistence and long-horizon persistence remain separate isolated proof stores rather than one unified cognitive runtime checkpoint, and both remain non-authoritative descriptive evidence only.
99. Final v0.1 closure does not change the main brain schema, does not persist bounded imagination, and does not allow replayed or restored state to synthesize factual evidence or authority.
100. Missing, corrupt, or incompatible standalone proof persistence must return explicit non-proof fallback rather than partial recovery or synthesized acceptance.
101. Deferred post-v0.1 research remains outside the closure claim, including learned context-similarity weights, semantic abstraction beyond grounded exact context signatures, and simultaneous multiple needs.
102. Final v0.1 closure does not authorize integration, production action authority, scheduler authority, executor authority, recommendation authority, selection authority, promotion authority, live Nursery authority, or SQLite cognition.
103. Any integration-facing observer or shadow artifacts are boundary-enforcement evidence only and do not constitute approval for production integration.
104. Comparison with the original SeedMind master plan and an explicit retain, adapt, partially integrate, or do-not-integrate decision must precede any integration implementation.
